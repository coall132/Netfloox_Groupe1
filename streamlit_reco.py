...
def film_le_plus_proche(a, df,c, n=5):  # Finding the index of the given film on the df global
    ...
    cos1=[item for sublist in cos for item in sublist]
...
st.title('Prédiction popularité')
st.subheader('Entrez les données :')
st.subheader("Pour les noms, ne metter pas d'espaces entre nom et prénom")
directors = st.text_input('directors')
writers = st.text_input('writers')
genres_type = st.text_input('genres_type')
actor = st.text_input('actor')
originaltitle = st.text_input('title')
    
startyear = st.number_input('startyear')
runtimeminutes = st.number_input('runtimeminutes')
numvotes = st.number_input('numvotes')
